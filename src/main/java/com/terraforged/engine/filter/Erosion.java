...
public class Erosion implements Filter
{
    ...
    @Override
    public void apply(final Filterable map, final int regionX, final int regionZ, final int iterationsPerChunk) {
        final int chunkX = map.getBlockX() >> 4;
        final int chunkZ = map.getBlockZ() >> 4;
        final int lengthChunks = map.getSize().total >> 4;
        final int borderChunks = map.getSize().border >> 4;
        final Size size = map.getSize();
        ...
        for (int lifetime = 0; lifetime < this.maxDropletLifetime; ++lifetime) {
            ...
            if (sediment > sedimentCapacity || deltaHeight > 0.0f) {
                ...
                for (int brushPointIndex = 0; brushPointIndex < this.erosionBrushIndices[dropletIndex].length; ++brushPointIndex) {
                    ...
                    final float deltaSediment = Math.min(cell.value, weighedErodeAmount);
    ...
    private static class TerrainPos
    {
        ...
        private TerrainPos at(final Cell[] nodes, final int mapSize, final float posX, final float posY) {
            ...
            final float heightNW = nodes[nodeIndexNW].value;
            final float heightNE = nodes[nodeIndexNW + 1].value;
            final float heightSW = nodes[nodeIndexNW + mapSize].value;
            final float heightSE = nodes[nodeIndexNW + mapSize + 1].value;
