...
public class LongIterSet
{
    ...
    public long nextLong() {
        while (++this.index < this.size) {
            final long value = this.order[this.index];
    ...
    private static long[] ensureCapacity(final long[] backing, final int index) {
        if (backing.length <= index) {
            final long[] next = new long[backing.length << 1];
