...
public class PerlinNoise extends NoiseSource
{
    ...
    public PerlinNoise(final Builder builder) {
        super(builder);
    ...
    public boolean equals(final Object o) {
        ...
        final PerlinNoise that = (PerlinNoise)o;
    ...
    private static float signal(final int octaves) {
        final int index = Math.min(octaves, PerlinNoise.signals.length - 1);
        return PerlinNoise.signals[index];
    }
    
    public static DataSpec<PerlinNoise> spec() {
        return NoiseSource.specBuilder("Perlin", PerlinNoise.class, PerlinNoise::new).build();
    }
